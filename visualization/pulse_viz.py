...
from core.grape_core import two_pi, make_hamiltonian, step_data  # if you defined it there, otherwise use 2*np.pi
...
PULSE_GATE_MAP = {
    "u_opt_main.npy": ("U_opt", get_g6_state_pairs, ["|g,0⟩ → |g,6⟩"]),
    "u_enc_main.npy": ("U_enc", get_encode_state_pairs, ["|g,0⟩ → |+Z_L⟩", "|e,0⟩ → |-Z_L⟩"]),
    "u_dec_main.npy": ("U_dec", get_decode_state_pairs, ["|+Z_L⟩ → |g,0⟩", "|-Z_L⟩ → |e,0⟩"]),
    "u_X_main.npy":   ("U_X",   get_logical_X_state_pairs, ["|+Z_L⟩ → |-Z_L⟩", "|-Z_L⟩ → |+Z_L⟩"]),
    "u_Y_main.npy":   ("U_Y",   get_logical_Y_state_pairs, ["|+Z_L⟩ → -i|-Z_L⟩", "|-Z_L⟩ → +i|+Z_L⟩"]),
    "u_Z_main.npy":   ("U_Z",   get_logical_Z_state_pairs, ["|+Z_L⟩ → |+Z_L⟩", "|-Z_L⟩ → -|-Z_L⟩"]),
    "u_H_main.npy":   ("U_H",   get_logical_H_state_pairs, ["|+Z_L⟩ → (|+Z_L⟩+|-Z_L⟩)/√2", "|-Z_L⟩ → (|+Z_L⟩-|-Z_L⟩)/√2"]),
    "u_T_main.npy":   ("U_T",   get_logical_T_state_pairs, ["|+Z_L⟩ → |+Z_L⟩", "|-Z_L⟩ → e^{iπ/4}|-Z_L⟩"]),
    "u_I_main.npy":   ("U_I",   get_identity_state_pairs, ["|+Z_L⟩ → |+Z_L⟩", "|-Z_L⟩ → |-Z_L⟩"]),
}
