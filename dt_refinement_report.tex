...
\begin{abstract}
The GRAPE time step is reduced by a factor of ten with a two-stage schedule.
...
\begin{equation}
  N \;\rightarrow\; sN, \qquad dt \;\rightarrow\; dt/s.
\end{equation}
...
The method used here is a two-stage split:
\begin{enumerate}
  \item \textbf{Stage~1.} Fit initial waveform on the coarse grid with multi-truncation
...
\end{enumerate}
Results are shown for two pulses: $U_{\mathrm{opt}}$
...
list (e.g.\ $\{22,24,26\}$), along with the discrepancy penalty so the
...
$n_c=24$, Stage~1 vs Stage~2 (initial state $\lvert g,0\rangle$; encode uses
the ground branch). Table~\ref{tab:traj} has the scalar metrics. Fock
heatmaps $P(n,t)$ and Stage~1 vs Stage~2 waveform overlays are in
Appendix~\ref{app:extra}.

\begin{figure}[h]
\centering
\includegraphics[width=0.98\textwidth]{figures/dt_refine_photon_trajectory.pdf}
...
The time step is reduced by a factor of ten with multi-truncation fitting on
...
%----------------------------------------------------------------------
\appendix
\section{Fock heatmaps and waveforms}
\label{app:extra}

Figure~\ref{fig:fock} shows $P(n,t)$ for Stage~1 vs Stage~2 at $n_c=24$
(red: $\langle n\rangle(t)$). Figures~\ref{fig:wave_opt}
and~\ref{fig:wave_enc} overlay the four control channels before and after
Stage~2 (amplitudes in MHz).

\begin{figure}[h]
\centering
\includegraphics[width=0.98\textwidth]{figures/dt_refine_fock_heatmap.pdf}
\caption{Fock population $P(n,t)$ at $n_c=24$. Top row: $U_{\mathrm{opt}}$;
bottom row: $U_{\mathrm{enc}}$ ($\lvert g,0\rangle$ branch). Left column:
Stage~1; right column: Stage~2. Red curve: $\langle n\rangle(t)$.}
\label{fig:fock}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{figures/dt_refine_waveform_opt.pdf}
\caption{Control waveforms for $U_{\mathrm{opt}}$: Stage~1 ($dt=2\,\mathrm{ns}$)
vs Stage~2 ($dt=0.2\,\mathrm{ns}$). Solid: $I$ channels; dashed: $Q$.}
\label{fig:wave_opt}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{figures/dt_refine_waveform_enc.pdf}
\caption{Control waveforms for $U_{\mathrm{enc}}$: Stage~1 ($dt=2\,\mathrm{ns}$)
vs Stage~2 ($dt=0.2\,\mathrm{ns}$). Solid: $I$ channels; dashed: $Q$.}
\label{fig:wave_enc}
\end{figure}
